...
struct Timeline {
    ...
    mutating func appendAction(_ action: Action) {
        ...
        print(action.description(in: game))
    }
}
...
enum Action {
    ...
    func description(in game: Game?) -> String {
        ...
        case .endFrame(let index, let frame):
            ...
            let (winnerScore, loserScore) = position == .A ? (frame.playerOneScore, frame.playerTwoScore) : (frame.playerTwoScore, frame.playerOneScore)
            return """
        ...
        case .endGame:
            return """
            Started game \(game.playerOne.name) vs \(game.playerTwo.name)
            \n
            """
        case .pot(let ball):
            return """
